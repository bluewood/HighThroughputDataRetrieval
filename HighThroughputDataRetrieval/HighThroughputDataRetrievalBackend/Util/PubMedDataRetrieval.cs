...
namespace HighThroughputDataRetrievalBackend.Util
{
  ...
  // constructor: initialize all properties 
        public PubMedDataRetrieval()
        {
            ...
            KeywordDataTable.Columns.Add("KeywordName");
            ...
            OrganismDataTable.Columns.Add("OrganismName");
            ...
            ProteinsDataTable.Columns.Add("ProteinName");
        ...
        override public int GetCount(string protein, string organism, string  keyword)
        {
            ...
            #region Members
            const string pubmedSearchPrefix = "http://eutils.ncbi.nlm.nih.gov/entrez/eutils//esearch.fcgi?db=pubmed&retmax=5&organism=";

            // combine proteins and keyword
            string terms = string.Format("{0},{1}", protein, keyword);

            // make url
            string assembleUrl = string.Format("{0}{1}&term={2}",
                                            pubmedSearchPrefix,
                                            organism,
                                            terms);
            #endregion
            ...
            {
                Console.WriteLine(Resources.PubMedDataRetrieval_GetCount_Exception_caught_while_retrieving_results_from_NCBI__ +
                    exc);
            }
            ...
            // Checking already searched is here fine for update later 
            string name = protein + organism + keyword;
            int count = int.Parse(doc.GetElementsByTagName("Count")[0].InnerText);
            ...
            // T_Protein 
            DataRow rowPro;
            if (!(Dictionary.TryGetValue(protein, out rowPro)))
            {
                rowPro = ProteinsDataTable.NewRow();
                rowPro["ProteinID"] = ProteinsDataTable.Rows.Count + 1;
                rowPro["ProteinName"] = protein;

                ProteinsDataTable.Rows.Add(rowPro);
                Dictionary.Add(protein, rowPro);
            }

            // T_Organism
            DataRow rowOrg;
            if (!(Dictionary.TryGetValue(organism, out rowOrg)))
            {
                rowOrg = OrganismDataTable.NewRow();
                rowOrg["OrganismID"] = OrganismDataTable.Rows.Count + 1;
                rowOrg["OrganismName"] = organism;

                OrganismDataTable.Rows.Add(rowOrg);
                Dictionary.Add(organism, rowOrg);
            }

                
            // T_Keyword
            DataRow rowKeyword;
            if (!(Dictionary.TryGetValue(keyword, out rowKeyword)))
            {
                ...
                Dictionary.Add(keyword, rowKeyword);
            ...
            rowQuery = QueryDataTable.NewRow();
            ...
            // T_QueryArticles
            XmlNodeList pmidList = doc.GetElementsByTagName("Id");
            foreach(XmlNode pmid in pmidList)
            {
                ...
                // add pmid into ld list and the row into datatable
        ...
        {               
           ...
           return QueryArticlesDataSet.Tables.Cast<DataTable>().Any(table => table.Rows.Count == 0) ? 
               null : QueryArticlesDataSet;
        }
